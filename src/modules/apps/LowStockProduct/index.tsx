...
import { dummyData as data } from "../dummydata/product";
...
const TableListing = () => {
  ...
  return (
    <AppsContainer
      title={"Low Stock Products" as string}
      isbutton
            ...
            <TableBody>
              {data.map((row, index) => {
                if (row.quantity < row.threshold) {
                  return (
                    <TableContent
                      key={index} // Add a unique key prop to each item for better performance
                      index={index}
                      row={row}
                ...
                return null; // Explicitly return null for items that don't meet the condition
              })}
