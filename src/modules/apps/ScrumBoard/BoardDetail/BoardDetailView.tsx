...
import { onAddNewList, onDeleteSelectedList, onEditBoardList, onUpdateCardCategory } from "../../../../toolkit/actions";
...
const BoardDetailView = (props: BoardDetailViewProps) => {
  ...
  const getCardById = (lane: CardListType, cardId: number) => lane.cards!.find((item) => item.id === cardId);
  ...
  const handleDragCard = (cardId: any, sourceLaneId: number, targetLaneId: number, position: number, cardDetails: CardType) => {
  ...
  return (
    ...
    >
      <Board
        ...
        onLaneDelete={(laneId: number) => dispatch(onDeleteSelectedList(boardDetail.id!, laneId))}
