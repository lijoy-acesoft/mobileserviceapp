...
export const getFormattedDate = (dateObject?: dayjs.Dayjs | string, format = "DD MMM YYYY") => {
  if (dateObject) return dayjs(dateObject).format(format);
  return "";
};

export const getFormattedDateTime = (value = 0, unit: any = "days", format = "DD MMM YYYY") => {
  if (value === 0) {
    return dayjs().format(format);
  } else {
